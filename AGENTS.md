...
**A key refusal is not an outage.** The backend's `/auth/exchange` answers `403` with `code: "key_refused"` when the gateway's mint endpoint rejects the *user* (a 400/422 on the mint call — today an email domain outside the gateway's allowlist, e.g. a sub-domain of a listed one). Gateway 5xx and the backend's own credential failures stay `502`. `backend.ts#fetchApiKey` throws a `BackendError` carrying the backend's `reason` and `code`, and `isKeyRefusal` is what `FetchApiKey` branches on: a refusal renders the gateway's sentence with **no retry affordance** and Enter goes straight to the manual-credentials step, because nothing the user can do on this machine changes the answer. The `code` string is a wire contract with `codev-backend/src/litellm.ts` — rename it in both or the hub silently falls back to the generic retry prompt. `FetchApiKey` also switches its key listener off once it has succeeded or handed off (`isActive`): the parent keeps the Step mounted as history, and a live listener there used to answer every later Enter by calling `onFallback` again.
